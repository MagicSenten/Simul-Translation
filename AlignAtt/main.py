...
def parse_args():
    ...
    parser.add_argument("--dataset_path",
                        default="../Data_preparation/prefixes_dataset.jsonl",
                        type=str, help="Path to the jsonl file with data.")
    parser.add_argument("--local_agreement_length", type=int, default=0,
                        help="Number of next tokens it must agree with the previous theory in")
    parser.add_argument("--skip_l", type=int, default=0,
                        help="Number of last positions in attention_frame_size to ignore")
    parser.add_argument("--layers", type=int, nargs='+', default=[4],
                        help="List of layer indices")
    parser.add_argument("--top_attentions", type=int, default=0,
                        help="Top attentions to use, set to zero to disable alignatt.")
    parser.add_argument("--attention_frame_size", type=int, default=20,
                        help="The excluded frame of last positions size")
    parser.add_argument("--count_in", type=int, default=7,
                        help="How many values in the top_attentions must be in attention_frame_size from end for the position to be bad.")
    parser.add_argument("--heads", type=int, nargs='+', default=list(range(6)),
                        help="List of attention heads")
    parser.add_argument("--device", type=str, default="cuda",
                        help="Device to use")
    parser.add_argument("--words_per_prefix", type=int, default=2,
                        help="Words per prefix shown")
    parser.add_argument("--forced_bos_token_text", type=str, default=None,
                        help="Forced BOS token text")
    parser.add_argument("--model_id", type=int, default=id, help="Model ID")
    parser.add_argument("--num_beams", type=int, default=10,
                        help="Setting the num_beams to a multiple of three turns on diverse beam search with num_beams//3 groups.")
    parser.add_argument("--src_key", type=str, default=keys[0],
                        help="Source key")
    parser.add_argument("--tgt_key", type=str, default=keys[1],
                        help="Target key")
...
def trim_to_last_word(self, hypothesis, tokenizer):
    ...
    while last_valid > 0 and tokenizer.decode(
            [hypothesis[last_valid - 1]]).startswith("▁"):
        print(
...
def local_agreement(self, states, new_hypothesis, segment_finished, tokenizer):
    ...
    if not segment_finished:
        stable_len = 0
        for stable_len, (a, b) in enumerate(
                zip(states.hypothesis, new_hypothesis)):
            if a != b:
                break
...
def make_alignments(datap, args):
    ...
    for x in range(0, len(src_words), args.words_per_prefix):
        alignments.append(
            (" ".join(src_words[0:x + args.words_per_prefix]), tgt))
...
def translate(model, tokenizer, input_text, stable_theory, args,
              verbose=False):
    '''
    ...
    decoder_input_ids = torch.tensor(
        tokenizer.convert_tokens_to_ids(stable_theory)).unsqueeze(0) if len(
        stable_theory) > 0 else None
    """
    ...
    """
    config = GenerationConfig(num_beams=args.num_beams,
    ...
    outputs = model.generate(input_ids=input_ids.to(args.device),
                             ...
                             output_attentions=True, max_new_tokens=20,
                             generation_config=config, renormalize_logits=True,
                             forced_bos_token_id=tokenizer.convert_tokens_to_ids(
                                 args.forced_bos_token_text) if args.forced_bos_token_text is not None else None)
    ...
    if args.top_attentions > 0 and not is_sent_end and not decoder_input_ids is None:
        ...
        if verbose:
            visualize_attention(input_ids[0],
                                output_ids[decoder_input_ids.shape[1]:],
                                attentions, tokenizer, args)
        alignatt_result = decoder_input_ids.shape[1] + alignatt(attentions,
                                                                args)
    else:
        alignatt_result = len(output_ids)

    decoded = tokenizer.decode(output_ids, skip_special_tokens=True)
    decoded_align_att = tokenizer.decode(output_ids[:alignatt_result],
...
def analyze_dataset(args):
    ...
    tokenizer = AutoTokenizer.from_pretrained(names[args.model_id],
                                              src_lang="ces_Latn",
                                              tgt_lang="eng_Latn")
    model = AutoModelForSeq2SeqLM.from_pretrained(names[args.model_id],
                                                  attn_implementation="eager").to(
        args.device)
    first = True
    ...
    for x in prefixes[:10]:
        ...
        stable_theory = tokenizer.tokenize(helper_text_en + x[-1][1])[
                        :lhten_tok + int(start * 2.5)]
        previous_theory = stable_theory
        for t in range(1, len(words)):
            partial_input_text = " ".join(words[:t + 1])
            if t >= wait_for:
                new_theory = translate(model, tokenizer,
                                       helper_text + partial_input_text,
                                       stable_theory, args, False)
                ...
                if args.local_agreement_length > 0:
                    for i in range(len(stable_theory), stop):
                        if any([new_theory[j] != previous_theory[j] for j in
                                range(i, min(stop,
                                             i + args.local_agreement_length))]) or len(
                                new_theory) > 500:
            ...
            metric.update(partial_input_text, full_input_text,
                          to_string(stable_theory), gold_text, tokenizer)
