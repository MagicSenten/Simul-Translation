...
class SentenceSegmenter:
    ...
    sep = 'Ĺ˝ĹľĹ˝ĹľSentenceSeparatorĹ˝ĹľĹ˝Ĺľ'  # string that certainly won't be in src or target
    latin_terminals = '!?.'
    jap_zh_terminals = 'ă€‚ďĽďĽź'
...
class SimulLLM:
    ...
    def __init__(self, llmtrans, min_len=0, chunk=1, trimming="sentences", language="ja", init_src="", init_tgt=""):
        ...
        self.buffer = ParallelTextBuffer(self.llmtranslator.tokenizer, self.llmtranslator.max_tokens_to_trim,
                                         trimming=trimming, init_src=init_src, init_tgt=init_tgt)
...
parser.add_argument('--input-instance', type=str, default=None,
                    help="Filename of instances to simulate input. If not set, txt input is read from stdin.")
...
default_inits_tgt = {
    'de': "Bitte schĂ¶n, Sie kĂ¶nnen mit Ihrer PrĂ¤sentation beginnen, wir sind bereit.",
    'ja': "ă©ă†ăžă€ă—ă¬ă‚Ľăłă†ăĽă‚·ă§ăłă‚’ĺ§‹ă‚ă¦ăŹă ă•ă„ă€‚",
    # # Please go ahead and start your presentation.  # this is in English
    'zh-tr': "č«‹çąĽçşŚďĽŚć‚¨ĺŹŻä»Ąé–‹ĺ§‹ć‚¨çš„ç°ˇĺ ±ďĽŚć‘ĺ€‘ĺ·˛ç¶“ćş–ĺ‚™ĺĄ˝äş†ă€‚",
    'zh-sim': "čŻ·ĺ§ďĽŚä˝ ĺŹŻä»ĄĺĽ€ĺ§‹ĺŹ‘č¨€äş†ďĽŚć‘ä»¬ĺ·˛ç»Źĺ‡†ĺ¤‡ĺĄ˝äş†ă€‚",
    'cs': "ProsĂ­m, mĹŻĹľete zaÄŤĂ­t s prezentacĂ­, jsme pĹ™ipraveni.",
}
parser.add_argument('--init_prompt_src', type=str, default=None,
                    help='Init translation with source text. It should be a complete sentence in the source language. '
                         'It can be context specific for the given input. Default is ')
parser.add_argument('--init_prompt_tgt', type=str, default=None,
                    help='Init translation with this target. It should be example translation of init_prompt_src. '
                         ' There is default init message, depending on the language.')

parser.add_argument('--len-threshold', type=float, default=None,
                    help='Ratio of the length of the source and generated target, in number of sentencepiece tokens. '
...
lan_thresholds = {
    'de': 1.3,
    # 12751/9817  ... the proportion of subword tokens for ACL6060 dev de vs. en text, for EuroLLM-9B-Instruct tokenizer
    'ja': 1.34,  # 13187/9817
    'zh': 1.23,  # 12115/9817
...
parser.add_argument("--max-context-length", type=int, default=4096,
                    help="Maximum number of tokens in the model to use.")

parser.add_argument("--buffer_trimming", type=str, default="sentences", choices=["segments", "sentences"],
...
if args.init_prompt_src is None:
    ...
    if args.init_prompt_tgt is None:
        ...
        if args.lan == "ja":
            ...
            print(
                "WARNING: Default init_prompt_src not set and language is Japanese. The init_src prompt changed to be more verbose.",
                file=sys.stderr)
    else:
        print("WARNING: init_prompt_tgt is used, init_prompt_src is None, the default one. It may be wrong!",
              file=sys.stderr)
        init_tgt = args.init_prompt_tgt
else:
    init_src = args.init_prompt_src.split()
    if args.init_prompt_tgt is None:
        print("WARNING: init_prompt_src is used, init_prompt_tgt is None, so the default one is used. It may be wrong!",
              file=sys.stderr)
        init_tgt = default_inits_tgt[args.lan]
    else:
        init_tgt = args.init_prompt_tgt
...
if args.language_specific_len_threshold:
    if args.len_threshold is not None:
        print("ERROR: --len-threshold is set, but --language-specific-len-threshold is also set. Only one can be used.",
              file=sys.stderr)
        sys.exit(1)
    else:
    ...
    def yield_input():
        for line in sys.stdin:
            ...
            if text[0] != " ":
                ...
                yield (ts, beg, end,
                       " " + first)  # marking the first word with " ", so that it can be later detected and inserted as suffix
            else:
                words = text.split()
            for w in words:
                yield (ts, beg, end, w)
