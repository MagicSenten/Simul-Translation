...
def translate_LLM(model, tokenizer, input_text, stable_theory, args, computation_stats, verbose=False):
    ...
    Args:
        ...
        stable_theory (list): A list of tokens representing the stable hypothesis.
        args (argparse.Namespace): Parsed command-line arguments.
        computation_stats (dict): A dictionary to store computation statistics.
        verbose (bool, optional): If True, enables verbose output. Defaults to False.

    Returns:
        list: A list of tokens representing the translated output.
    """
    ...
    config = GenerationConfig(
        ...
        bad_words_ids=[tokenizer.encode(x) for x in bad_words]
    )
    outputs = model.generate(
        input_ids=all_input_ids.to(args.device),
        generation_config=config,
        return_dict_in_generate=True,
        output_attentions=True,
        max_new_tokens=min(2, input_ids.shape[1] * 1.5 - len(stable_theory)),
        renormalize_logits=True,
        forced_bos_token_id=tokenizer.convert_tokens_to_ids(args.forced_bos_token_text) if args.forced_bos_token_text is not None else None,
    ...
    if args.top_attentions > 0 and decoder_input_ids is not None and len(ca[1:]) > 0:
...
def translate(model, tokenizer: PreTrainedTokenizerBase, input_text, stable_theory, computation_stats, args, verbose=False):
    ...
    outputs = model.generate(
        ...
        decoder_input_ids=decoder_input_ids.to(args.device) if decoder_input_ids is not None else None,
        return_dict_in_generate=True,
        output_attentions=True,
        max_new_tokens=10,
        generation_config=config,
    ...
    if args.top_attentions > 0 and decoder_input_ids is not None and len(ca) > 1:
